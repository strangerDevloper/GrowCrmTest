...
const Topbar = ({ view, setView, setIsFiltered, isFiltered, showAddClient, onAddClick }) => {
  ///////////////////////////////////////// VARIABLES ///////////////////////////////////////////////////
  const { pathname } = useLocation();
  const dispatch = useDispatch();
  const pathArr = pathname.split("/").filter((item) => item != "");
  const showClientTopBar = pathArr.includes("clients");
  const showEmployeeTopBar = pathArr.includes("employees");
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="bg-[#ebf2f5] hover:bg-[#dfe6e8] p-1 pl-2 pr-2 rounded-md w-48">
            <FormControl>
              <Input
                name="search"
                placeholder={`Search ${showClientTopBar ? 'Clients' : 'Employees'}`}
          ...
          {(showEmployeeTopBar || showAddClient) && (
            <Tooltip title={`Add New ${showClientTopBar ? 'Client' : 'Employee'}`} placement="top" arrow>
              <div onClick={showClientTopBar ? onAddClick : handleCreateopen("body")}>
